...
import 'package:kg_students/Components/check-answer-button.dart';
import 'package:kg_students/Components/hint-text.dart';
import 'package:kg_students/Components/math-choice-button.dart';
...
class _MathPageState extends State<MathPage> {
  ...
  var _r = new List.generate(
      2,
      (index) =>
          new Random().nextInt((10))); // random 2 numbers to generate equation
  var _postion = new List.generate(
      1,
      (index) => new Random().nextInt(
          (100))); // to make random postison for true and false answers
  var _operations = new List.generate(
      1,
      (index) => new Random()
          .nextInt((2))); //random number to decide if the operation is + or -
  var _notT = new List.generate(
      1,
      (index) =>
          new Random().nextInt((20))); //random number to generate false answer
...
//! check ,show result  and generate a new equation
  void onClick() {
    ...
    setState(() {
      _r = new List.generate(2, (index) => new Random().nextInt((10)));
      _postion = new List.generate(1, (index) => new Random().nextInt((100)));
      _operations = new List.generate(1, (index) => new Random().nextInt((2)));
      _notT = new List.generate(1, (index) => new Random().nextInt((20)));
      _torf = false;
    });
  }
